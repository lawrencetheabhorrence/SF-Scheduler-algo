...
ga_params = {
    ...
    'fitness_src': root + big_folder + f"_{gen}_{date}_fitness.csv",
...
def __main__():
    ...
    with open(f"{gen}_{date}.txt", "w") as f:
    ...
    for i, day in enumerate(df):
        day.to_html(root + big_folder + f'_{gen}_{date}_result_{i}.html', escape=False)
        day.to_csv(root + big_folder + f'_{gen}_{date}_result_{i}.csv')
        global days
        days = i + 1
__main__()
